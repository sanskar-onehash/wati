...
frappe.ui.form.on("Notification", {
  ...
  wati_map_fields: function (frm) {
    ...
    meta.fields.forEach((e) => {
      if (
        e.options == "Phone" ||
        e.fieldname == "contact_phone" ||
        e.fieldname == "contact_mobile"
      ) {
    ...
    var d = new frappe.ui.Dialog({
      ...
      primary_action: function (values) {
        if (
          frm.doc.message != "" &&
          frm.doc.message != "Add your message here"
        ) {
          frappe.confirm(
            "This will overwrite the message. Are you sure you want to proceed?",
            () => {
        ...
        .then((data) => {
          ...
          if (counter == 1) {
            ...
            data.whatsapp_map.forEach((e) => {
              ...
              d.fields_dict[e.field_name].input.onchange = function () {
                if (
                  ...
                  this.value &&
                  this.value.replace(", ", "") == "Print Format"
                ) {
            ...
            if (!["text", "", undefined, null].includes(data.header_type)) {
              header_html =
                data.header_type.charAt(0).toUpperCase() +
                data.header_type.slice(1) +
                ` Attachment: <a href="` +
                data.header_link +
                `">` +
                data.header_link +
...
function verify(d, context, data, header_html, data_dict) {
  for (const [key, value] of Object.entries(context)) {
    if (d.get_field(key).input.value.replace(", ", "") == "Attachment") {
      if (
        d.get_field(key + "_attachment").value == null ||
        d.get_field(key + "_attachment").value.length == 0
      ) {
        return;
      } else if (
        d.get_field(key + "_attachment").value &&
        d.get_field(key + "_attachment").value.includes("/private/")
      ) {
        d.get_field(key + "_attachment").value = "";
        ...
        d.get_field(key + "_attachment").value &&
        d.get_field(key + "_attachment").value.includes("https://")
      ) {
      ...
      d.get_field(key).input.value.replace(", ", "") == "Print Format"
    ) {
      frappe.call({
        method: "wati.users.get_attach_link",
        args: {
          doc: { doctype: cur_frm.doc.doctype, docname: cur_frm.doc.name },
          print_format: d.get_field(key + "_print_format").value,
        },
        callback: (r) => {
